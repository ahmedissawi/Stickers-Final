...
//        let trayCompliantImageData: ImageData = try
//            ImageData.imageDataIfCompliant(contentsOfFile: trayImageFileName, isTray: true)\
        let dataDecoded : Data = Data(base64Encoded: trayImageFileName, options: .ignoreUnknownCharacters)!

        self.trayImage = ImageData(data: dataDecoded , type: ImageDataExtension(rawValue: "png")!)
    ...
    func sendToWhatsApp(completionHandler: @escaping (Bool) -> Void) {
        StickerPackManager.queue.async {
            ...
            json["ios_app_store_link"] = "https://itunes.apple.com/app/id1462045333?mt=8"
            json["android_play_store_link"] = "https://play.google.com/store/apps/details?id=com.imNMSH.StickerFree"
